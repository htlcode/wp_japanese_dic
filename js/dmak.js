		...
		if(!this.options.loadCode){
			...
			if (!this.options.skipLoad) {
				var loader = new DmakLoader(this.options.uri),
					...
					loader.loadByCode(text, function (data) {
						self.prepare(data);
						// Execute custom callback "loaded" here
						self.options.loaded(self.kanjis);
...
;(function () {
	...
	DmakLoader.prototype.loadByCode = function (code, callback) {
		var paths = [];
		...
		xhr.onreadystatechange = function () {
			if (xhr.readyState === 4) {
				if (xhr.status === 200) {
					callbacks.done(parseResponse(xhr.response, code));
				} else {
